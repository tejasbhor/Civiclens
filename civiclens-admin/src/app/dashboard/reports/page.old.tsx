    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          toast.textContent = `Status updated to ${toLabel(newStatus)}`;
      ...
      onConfirm: async (password: string) => {
        try {
          ...
          if (result.successful > 0) {
            const toast = document.createElement('div');
            toast.className = 'fixed top-4 right-4 bg-green-600 text-white px-4 py-2 rounded-lg shadow-lg z-50';
            toast.textContent = `Successfully assigned ${result.successful} report(s) to ${department.name}`;
            document.body.appendChild(toast);
      ...
      onConfirm: async (password: string) => {
        try {
          ...
          if (result.successful > 0) {
            const toast = document.createElement('div');
            toast.className = 'fixed top-4 right-4 bg-green-600 text-white px-4 py-2 rounded-lg shadow-lg z-50';
            toast.textContent = `Successfully assigned ${result.successful} report(s) to ${officer.full_name || officer.email}`;
            document.body.appendChild(toast);
            setTimeout(() => document.body.removeChild(toast), 3000);
          }
    ...
    setPasswordDialog({
      ...
      onConfirm: async (password: string) => {
        try {
          ...
          if (result.successful > 0) {
            const toast = document.createElement('div');
            const bgColor = bulkSeverity === 'critical' ? 'bg-red-600' : 
                           bulkSeverity === 'high' ? 'bg-orange-600' : 
                           'bg-green-600';
            toast.className = `fixed top-4 right-4 ${bgColor} text-white px-4 py-2 rounded-lg shadow-lg z-50`;
            toast.textContent = `Successfully updated ${result.successful} report(s) to ${toLabel(bulkSeverity)} severity`;
            document.body.appendChild(toast);
            setTimeout(() => document.body.removeChild(toast), 3000);
          }
    ...
    const run = async () => {
      try {
        ...
        try {
          ...
          // For severity counts, we need to exclude closed/resolved/rejected reports
          // This is a client-side workaround - backend should handle this properly
          ...
          const [pend, needsRev, assigned, prog, resv] = await Promise.all([
            reportsApi.getReports({ page: 1, per_page: 1, status: ReportStatus.PENDING_CLASSIFICATION }),
            reportsApi.getReports({ page: 1, per_page: 1, needs_review: true }),
            reportsApi.getReports({ page: 1, per_page: 1, status: ReportStatus.ASSIGNED_TO_OFFICER }),
            reportsApi.getReports({ page: 1, per_page: 1, status: ReportStatus.IN_PROGRESS }),
            reportsApi.getReports({ page: 1, per_page: 1, status: ReportStatus.RESOLVED }),
          ]);
          
          setStats({
            total: allReports.total || 0,
            pending_classification: pend.total || 0,
            needs_review: needsRev.total || 0,  // Count of reports needing review
            ...
            high: activeReports.filter(r => r.severity === 'high').length,
          });
        }
      } catch (error) {
